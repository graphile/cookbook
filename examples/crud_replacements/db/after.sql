...
create function create_my_table(my_table my_table) returns my_table as $$
  insert into my_table (name, description)
    values (create_my_table.my_table.name, create_my_table.my_table.description)
    returning *;
  -- Or, if you prefer:
  --   insert into my_table (name, description) values ($1.name, $1.description) returning *;
...
create function update_my_table_by_id(my_table_patch my_table, id int) returns my_table as $$
  update my_table
    set name = my_table_patch.name, description = my_table_patch.description
    where my_table.id = update_my_table_by_id.id
    returning *;
$$ language sql volatile set search_path from current strict;
...
-- Delete mutation just needs to know what to delete
create function delete_my_table_by_id(id int) returns my_table as $$
  delete from my_table
    where my_table.id = delete_my_table_by_id.id
    returning *;
$$ language sql volatile set search_path from current strict;